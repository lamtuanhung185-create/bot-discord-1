...
class ScrcpyWirelessGUI:
    ...
    def setup_ui(self):
        ...
        # Checkbox tắt màn hình
        screen_option_frame = tk.Frame(device_frame)
        screen_option_frame.pack(fill=tk.X, pady=(10, 0))
        
        screen_checkbox = tk.Checkbutton(
            screen_option_frame,
            text="📱 Tắt màn hình điện thoại khi kết nối",
            variable=self.turn_off_screen,
            font=("Arial", 9),
            cursor="hand2",
            command=self.save_screen_preference
        )
        screen_checkbox.pack(anchor=tk.W)
